...
public:
  class Compressor {
  public:
    using Output = Channel<Compressed>;

    explicit Compressor(const Channel &input) : input_(input) {}

    auto Compress(Compression::Tp compression, std::uint64_t row_count, std::uint64_t column_count) const {
      return Create(
        compression,
        PSD::Compressor(input_.data).Compress(
          compression,
          row_count, 
          column_count
        )
...
class Channel<Compressed>::Decompressor {
public:
  using Output = Channel<Decompressed>;

  explicit Decompressor(const Channel &input) : input_(input) {}

  auto Decompress(std::uint64_t row_count, std::uint64_t column_count) {
    return Output(
...
public:
  ...
  class Compressor {
  public:
    using Output = ChannelData<DepthV, ColorV, Compressed>;

    explicit Compressor(const ChannelData &input) : input_(input) {}

    auto Compress(Compression::Tp compression) {
      Output output;

      auto array = Image::ChannelArrayConvertor(input_.buffer).Convert();
      for (auto index = 0u; 
                index < Color::ChannelCount<ColorV, Image::EnableAlpha>; 
                index++) {
        output.data_[index] = Image::SequenceConvertor(
          array[index]
...
template <Depth::Tp DepthV,
          Color::Tp ColorV>
class ChannelData<DepthV, ColorV, Compressed>::Decompressor {
public:
  ...
  auto Decompress() {
    ...
    for (auto index = 0u; 
              index < Color::ChannelCount<ColorV, Image::EnableAlpha>; 
              index++) {
      array[index] = Image::SequenceConvertor(
        static_cast<Image::Sequence>(
          input_.data_[index]
        )
      ).template Convert<
        DepthV, 
    ...
    return Image::ChannelArrayConvertor(array).Convert();
  }
private:
  const ChannelData &input_;
};
